# rsconnect::deployApp('~/Documents/Rpackages/gleon_predict_salt/Shiny/LakeChloride/')
...
# obs = read_csv('output_data_datout.csv') %>% 
...
# st_write(obs,'Data/observed.csv',layer_options = "GEOMETRY=AS_XY")
# 
...
# st_write(df.sf,'Data/predicted.csv',layer_options = "GEOMETRY=AS_XY")




obs = read_csv('Data/observed.csv') %>% st_as_sf(coords = c("X", "Y"), 
                                                  crs = 4326)
df.sf = read_csv('Data/predicted.csv') %>% st_as_sf(coords = c("X", "Y"), 
                                                    crs = 4326)
...
server <- function(input, output) {
    ...
    output$mymap <- renderLeaflet(({
        leaflet() %>% 
            ...
            addLegend("topright", pal = colorBin("viridis",df.sf$prediction.50), 
