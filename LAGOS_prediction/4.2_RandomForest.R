...
## load the model
# rf_model <- readRDS("./LAGOS_prediction/RFmodel_2020_03_27.rds")
...
ggsave('LAGOS_prediction/Figures/Figure_ValueSummary.png',width = 6,height = 6)
...
allLagos <- allLagos.raw %>%
  mutate(Month = 7) %>% 
  mutate_at(vars(LakeArea,WS.Area,wlconnections_allwetlands_count:RoadDistance, WinterSeverity),log01) %>%
  filter(!is.na(WS.Dev.Low))
...
allLagos.out = allLagos %>% 
  ...
  dplyr::select(lagoslakeid:MaxDepth,state_name,pred.mean:pred.95) %>% 
...
test = allLagos %>% filter(lagoslakeid == 63) %>% dplyr::select(colnames(rf_cov))
test
test.dat = dat_rf %>% filter(lagoslakeid == 63) %>% dplyr::select(colnames(rf_cov))
exp(predict(rf_model, data = test)$predictions)
exp(predict(rf_model, data = test.dat)$predictions)
...
dat.out.mean = dat.out %>% 
  ...
  mutate(residuals.50 = abs(pred.50 - medianCl))
...
ggplot(meanLakes_Lagos,aes(x = pred.50.x, y = pred.50.y)) + geom_point() +
